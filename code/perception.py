...
def color_range_thresh(img, rgb_lower_thresh=(0,0,0), rgb_higher_thresh=(255,255,255)):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
    # where threshold was met
    above_thresh = ((img[:,:,0] > rgb_lower_thresh[0]) & (img[:,:,0] < rgb_higher_thresh[0])) \
...
def perception_step(Rover):
    ...
    obstacle_thresh = color_range_thresh(warped, rgb_higher_thresh=(160,160,160)) 
    rock_thresh = color_range_thresh(warped, rgb_lower_thresh=(140,140,7),rgb_higher_thresh=(200,200,120))
    ...
    Rover.vision_image[:,:,2] = terrain_thresh
    # 5) Convert map image pixel values to rover-centric coords
    xpix_navigable, ypix_navigable = rover_coords(terrain_thresh)
